...
Assignment: Hangman Game, TASK D

CREATED A GAME CALLED HANGMAN
*/
...
bool addWord(string word, string definition, string pos) {
  if (getIndex(word) != -1 ||
      g_word_count == g_MAX_WORDS) { // if these statements are shown then it
                                     // wont print anything
    return false;
  }
...
bool editWord(string word, string definition, string pos) {
  int index =
      getIndex(word); // string that was using in part A and was countined
  if (index != -1) {
...
bool removeWord(string word) {
  int index = getIndex(word);
  if (index == -1) { // instead of using not equal to -1 we have to use equal to
                     // so it can go through the array
  ...
  while (true) {
    ...
    if (tries == 0) {
      cout << "The word is \"" << word
           << "\". Better luck next time! You're getting the ..ahem.. hang of "
              "it."
           << endl;
    } else {
      cout << "Congrats!!!" << endl;
